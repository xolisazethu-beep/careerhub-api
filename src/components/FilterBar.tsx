    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm text-ink outline-none transition placeholder:text-slate-400 focus:border-brand-500 focus:bg-white focus:ring-2 focus:ring-brand-500/30"
          />
        </div>
        ...
        <label className="flex items-center gap-2 text-sm text-slate-600">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white py-2.5 pl-3 pr-8 text-sm font-medium text-ink outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30"
          >
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {TYPE_OPTIONS.map((option) => {
          ...
          const label = option === "All" ? "All types" : EMPLOYMENT_TYPE_STYLES[option].label;
          return (
            <button
              key={option}
              type="button"
